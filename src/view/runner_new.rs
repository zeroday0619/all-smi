...
use crate::view::{data_collector::DataCollector, terminal_manager::TerminalManager, ui_loop::UiLoop};
...
pub async fn run_view_mode(args: &ViewArgs) {
    ...
    tokio::spawn(async move {
        ...
        if hosts.is_empty() && hostfile.is_none() {
            ...
            // Remote mode
            data_collector.run_remote_mode(args_clone, hosts, hostfile).await;
        }
    });
