...
pub fn print_help_popup<W: Write>(
    ...
    for y in 0..popup_height {
        queue!(
            stdout,
            crossterm::cursor::MoveTo(start_x, start_y + y as u16)
        )
        .unwrap();
        print_colored_text(
            stdout,
...
fn get_current_sort_status(sort_criteria: &crate::app_state::SortCriteria) -> String {
    match sort_criteria {
        crate::app_state::SortCriteria::Default => {
            "Current Sort: Default (Hostname then Index)".to_string()
        }
...
fn render_shortcut_item<W: Write>(stdout: &mut W, key: &str, desc: &str, is_header: bool) {
    if key.is_empty() {
        print_colored_text(stdout, "", Color::White, Some(Color::Black), None);
    } else if is_header {
        // This is a section header
...
fn draw_window_frame<W: Write>(stdout: &mut W, x: u16, y: u16, width: usize, height: usize) {
    ...
    for i in 1..height - 1 {
        ...
        queue!(
            stdout,
            crossterm::cursor::MoveTo(x + width as u16 - 1, y + i as u16)
        )
        .unwrap();
        print_colored_text(stdout, "║", Color::White, Some(Color::Black), None);
    }
